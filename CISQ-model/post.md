...
Although the model is much more complicated that the CIA-triad or even the STRIDE threat model, the CISQ-model presents an opportunity to apporach information security in a way that can start at low complexity with only the four basic qualities, and then guides the user towards deeper understanding of their own system by introducing or eliminating security qualities. This is done by the composition. If a basic quality is important or seen as irrelevant for the system this insight can be used to guide attention the higher level qualities composed form the basic quality.
